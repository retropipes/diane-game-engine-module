...
final class MacOSSandbox extends Sandbox {
	...
	private static final String LIBRARY_FALLBACK_DIR = "Library/Application Support/Sandboxes/"; //$NON-NLS-1$
	...
	// Constructor
	MacOSSandbox(final String name) {
	...
	protected String getDirectory(final SystemDir dir) {
		...
		case APPLICATION:
			...
			if (app == null) {
				app = this.getLibraryFallbackDirectory();
			}
			return app;
		case APPLICATION_SUPPORT:
			var appSupport = System.getProperty("ApplicationSupportDirectory"); //$NON-NLS-1$
			if (appSupport == null) {
				appSupport = this.getLibraryFallbackDirectory() + File.pathSeparator
		...
		case AUTOSAVED_INFORMATION:
			...
			if (autosave == null) {
				autosave = this.getLibraryFallbackDirectory() + File.pathSeparator + MacOSSandbox.AUTOSAVE_FALLBACK_DIR;
			}
			return autosave;
		case CACHES:
			var caches = System.getProperty("CachesDirectory"); //$NON-NLS-1$
			if (caches == null) {
				caches = this.getLibraryFallbackDirectory() + File.pathSeparator + MacOSSandbox.CACHES_FALLBACK_DIR;
			}
			return caches;
		case DOCUMENTS:
			var docs = System.getProperty("DocumentsDirectory"); //$NON-NLS-1$
			if (docs == null) {
				docs = this.getLibraryFallbackDirectory() + File.pathSeparator + MacOSSandbox.DOCUMENTS_FALLBACK_DIR;
			}
			return docs;
		case DESKTOP:
			var desktop = System.getProperty("DesktopDirectory"); //$NON-NLS-1$
			if (desktop == null) {
				desktop = this.getLibraryFallbackDirectory() + File.pathSeparator + MacOSSandbox.DESKTOP_FALLBACK_DIR;
			}
			return desktop;
		case DOWNLOADS:
			var downloads = System.getProperty("DownloadsDirectory"); //$NON-NLS-1$
			if (downloads == null) {
				downloads = this.getLibraryFallbackDirectory() + File.pathSeparator
		...
		case LIBRARY:
			...
			if (library == null) {
				library = this.getLibraryFallbackDirectory();
			}
			return library;
		case MOVIES:
			var movies = System.getProperty("MoviesDirectory"); //$NON-NLS-1$
			if (movies == null) {
				movies = this.getLibraryFallbackDirectory() + File.pathSeparator + MacOSSandbox.MOVIES_FALLBACK_DIR;
			}
			return movies;
		case MUSIC:
			var music = System.getProperty("MusicDirectory"); //$NON-NLS-1$
			if (music == null) {
				music = this.getLibraryFallbackDirectory() + File.pathSeparator + MacOSSandbox.MUSIC_FALLBACK_DIR;
			}
			return music;
		case PICTURES:
			var pictures = System.getProperty("PicturesDirectory"); //$NON-NLS-1$
			if (pictures == null) {
				pictures = this.getLibraryFallbackDirectory() + File.pathSeparator + MacOSSandbox.PICTURES_FALLBACK_DIR;
			}
			return pictures;
		case SHARED_PUBLIC:
			var sharedPublic = System.getProperty("SharedPublicDirectory"); //$NON-NLS-1$
			if (sharedPublic == null) {
				sharedPublic = this.getLibraryFallbackDirectory() + File.pathSeparator
		...
		case SYSTEM_APPLICATION:
			...
			if (sysApp == null) {
				sysApp = this.getLibraryFallbackDirectory();
			}
			return sysApp;
		case SYSTEM_APPLICATION_SUPPORT:
			...
			if (sysAppSupport == null) {
				sysAppSupport = this.getLibraryFallbackDirectory() + File.pathSeparator
						+ MacOSSandbox.APP_SUPPORT_FALLBACK_DIR;
			}
		...
		case SYSTEM_CACHES:
			...
			if (sysCaches == null) {
				sysCaches = this.getLibraryFallbackDirectory() + File.pathSeparator + MacOSSandbox.CACHES_FALLBACK_DIR;
			}
			return sysCaches;
		case SYSTEM_LIBRARY:
			var sysLibrary = System.getProperty("SystemLibraryDirectory"); //$NON-NLS-1$
			if (sysLibrary == null) {
				sysLibrary = this.getLibraryFallbackDirectory();
			}
			return sysLibrary;
		case SYSTEM_USER:
		...
		case USER_HOME:
			...
			return System.getProperty("user.home"); //$NON-NLS-1$
		default:
			return this.getLibraryFallbackDirectory();
		}
	}
