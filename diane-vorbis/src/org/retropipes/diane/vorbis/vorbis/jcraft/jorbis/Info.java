...
import org.retropipes.diane.vorbis.util.Util;
import org.retropipes.diane.vorbis.vorbis.jcraft.jogg.Buffer;
...
public class Info {
	...
	public void clear() {
		...
		for (var i = 0; i < this.maps; i++) { // unpack does the range checking
			FuncMapping.mapping_P[this.map_type[i]].free_info(this.map_param[i]);
		}
		this.map_param = null;
		for (var i = 0; i < this.times; i++) { // unpack does the range checking
			FuncTime.time_P[this.time_type[i]].free_info(this.time_param[i]);
		}
		this.time_param = null;
		for (var i = 0; i < this.floors; i++) { // unpack does the range checking
			FuncFloor.floor_P[this.floor_type[i]].free_info(this.floor_param[i]);
		}
		this.floor_param = null;
		for (var i = 0; i < this.residues; i++) { // unpack does the range checking
	...
	public String toString() {
		return "version:" + this.version + ", channels:" + this.channels + ", rate:" + this.rate + ", bitrate:"
				+ this.bitrate_upper + "," + this.bitrate_nominal + "," + this.bitrate_lower;
	}
